...
orb_publication_t *__orb_create_publication(const struct orb_metadata *meta) {
  return __orb_create_publication_multi(meta, nullptr);
}

orb_publication_t *__orb_create_publication_multi(const struct orb_metadata *meta,
...
bool __orb_destroy_publication(orb_publication_t **handle_ptr) {
...
bool __orb_publish(orb_publication_t *handle, const void *data) {
...
bool __orb_publish_anonymous(const struct orb_metadata *meta, const void *data) {
...
orb_subscription_t *__orb_create_subscription(const struct orb_metadata *meta) {
  return __orb_create_subscription_multi(meta, 0);
}
...
bool __orb_destroy_subscription(orb_subscription_t **handle_ptr) {
  ORB_CHECK_TRUE(handle_ptr && *handle_ptr, EINVAL, return false);
...
bool __orb_copy(orb_subscription_t *handle, void *buffer) {
...
bool __orb_copy_anonymous(const struct orb_metadata *meta, void *buffer) {
  ORB_CHECK_TRUE(meta, EINVAL, return false);
...
bool __orb_check_update(orb_subscription_t *handle) {
...
bool __orb_exists(const struct orb_metadata *meta, unsigned int instance) {
  ORB_CHECK_TRUE(meta, EINVAL, return false);
...
unsigned int __orb_group_count(const struct orb_metadata *meta) {
  ORB_CHECK_TRUE(meta, EINVAL, return false);

  unsigned int instance = 0;

  for (unsigned int i = 0; i < ORB_MULTI_MAX_INSTANCES; ++i) {
    if (__orb_exists(meta, i)) {
      ++instance;
    }
  }
  return instance;
...
int __orb_poll(struct orb_pollfd *fds, unsigned int nfds, int timeout_ms) {
