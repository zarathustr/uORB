...
class Subscription : internal::Noncopyable {
 ...
 protected:
   ...
   */
  virtual bool Updated() { return Subscribed() && __orb_check_update(handle_); }
 ...
 public:
  ...
  ~Subscription() { handle_ && __orb_destroy_subscription(&handle_); }
  ...
  bool Subscribed() {
    ...
    return (handle_ = __orb_create_subscription_multi(&meta, instance_));
  }
...
class SubscriptionData : public Subscription<T> {
 ...
 public:
  ...
  const Type* get_ptr() const {return &data_;}
